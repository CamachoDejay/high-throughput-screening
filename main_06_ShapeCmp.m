...
% for the plate/well
[ control.data, control.ids, control.vars ] = CellsCmp.load_Sdata( init_dir );
...
[cmp_model, cmp_data] = CellsCmp.svd_control(control,modelProps);

%% figure control
hfig = figure(25);
CellsCmp.shape_scat_plot( hfig, control)
...
[ sample.data, sample.ids, sample.vars, sample.dir, noDataIds ] = ...
                                           CellsCmp.load_Sdata( init_dir );

% Saving ids that have no data data in case is needed later on 
if ~isempty(noDataIds)
    ...
    writetable(noData,[sample.dir filesep 'NoDataWells_SHAPE.csv'])
end

% Saving control data in case is needed later on 
save([sample.dir filesep 'ControlDataShape.mat'],'control','cmp_data','var4model')

% compare to WT
[idx_95, idx_99, SPE_val, T2_val, cleanSample, missData, highDens] = ...
                                          CellsCmp.svd_cmp(sample,cmp_model);
writetable(missData,[sample.dir filesep 'MissingDataWells_SHAPE.csv'])
writetable(highDens,[sample.dir filesep 'HighDensityWells_SHAPE.csv'])                                      
...
vn = cmp_model.var_names;
for i = 1:length(var4model)
    tmp = vn{i};
    tmp(strfind(tmp,' ')) = [];
    tmp(strfind(tmp,'-')) = [];
    vn{i} = tmp;
...
writetable(outT1,[sample.dir filesep 'SampleVsControlSHAPE.csv'])
save([sample.dir filesep 'SHAPEcmpModel.mat'],'cmp_model')
...
hfig = figure(25);
CellsCmp.shape_scat_plot( hfig, control, sample, idx2show)
